...
def listar_permisos_por_cargo_service(id_cargo: int, db: Session):
    cargo = db.query(Cargo).options(joinedload(Cargo.permisos)).filter(Cargo.id_cargo == id_cargo).first()
    if not cargo:
        return []
    permisos = [
        PermisoSimple(
